...
namespace Digital_World
{
    ...
    {
        public static byte[][] TamerData = new byte[][] { new byte[] { 10, 2, 90, 80 }, new byte[] { 8, 1, 100, 90 }, new byte[] { 8, 1, 90, 100 }, new byte[] { 10, 1, 80, 90 } };
        /// <summary>
                   ...
                   using(MySqlDataReader dr = cmd.ExecuteReader())
                   {
                       if (dr.HasRows)
                       {
                           while (dr.Read())
                           {
                               ...
                               {
                                   Tamer.Equipment = new ItemList(13);
                               }
        ...
        public static int CreateCharacter(uint AcctId, int pos, int charModel, string charName, int digiModel)
        {
            ...
            try
            {
                using (MySqlConnection con = Connect())
                {
                    ...
                    qry.Add("characterPos", pos);

                    qry.Add("inventory", new ItemList(150).Serialize());
                    qry.Add("Warehouse", new ItemList(245).Serialize());
                    qry.Add("equipment", new ItemList(13).Serialize());
        ...
        public static bool VerifySecondPassword(uint acctId, string code)
        {
            bool allow = false;
            try
            {
                ...
                {
                    using (MySqlCommand cmd = new MySqlCommand(
                    "SELECT `secondpassword` from `acct` WHERE `accountId` = @acct",
                    con))
                    {
                        cmd.Parameters.AddWithValue("@acct", acctId);
                        using (MySqlDataReader read = cmd.ExecuteReader(System.Data.CommandBehavior.SingleRow))
                        {
                            ...
                            {
                                if (code.Equals((string)read["secondpassword"]))
                                    allow = true;
                            }
                        }
                    }
        ...
        public static string GetCharacterTimeStamp(string name)
        {
            string vreme = "1970-01-01 00:00:01";
            try
            {
                using (MySqlCommand cmd = new MySqlCommand(
                    "SELECT `maintenance` FROM `servers` WHERE `name`= @name", Connection))
                {
                    ...
                    using (MySqlDataReader data = cmd.ExecuteReader())
                    {
                        if (data.HasRows)
                        {
                            while (data.Read())
                            {
                                //vreme = data["maintenance"].ToString();
                            }
                        }
                    }
                }
